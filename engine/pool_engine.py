...
# ═══ Credit Delta Awareness ═══
_DELTA_FILE = os.path.join(os.path.expanduser('~'), '.wam', 'credit-deltas.json')

def _read_remote_in_use():
    """Read Hub's credit-deltas.json to get set of emails currently in use by other machines"""
    try:
        if not os.path.exists(_DELTA_FILE):
            return set()
        with open(_DELTA_FILE, 'r', encoding='utf-8') as f:
            data = json.load(f)
        now = time.time()
        remote = set()
        for email, entry in data.items():
            if entry.get('in_use_remote') and entry.get('decay_at', 0) > now:
                remote.add(email.lower())
        return remote
    except:
        return set()
...
class PoolEngine:
    ...
    def _load_blacklist(self):
        """Load genesis audit blacklist (Free/dead accounts to skip)."""
        bl_path = SCRIPT_DIR / '_free_blacklist.json'
        try:
            if bl_path.exists():
                d = json.loads(bl_path.read_text(encoding='utf-8'))
                s = set(e.lower() for e in d.get('free', []))
                s.update(e.lower() for e in d.get('dead', []))
                return s
        except Exception:
            pass
        return set()
    ...
    def reload(self):
        ...
        Pro Trial Only: skip Free and long-expired accounts."""
        self._pool.reload()
        now_ms = time.time() * 1000
        EXPIRED_GRACE_MS = 3 * 86400000  # 3 days
        ...
        with self.lock:
            ...
            for i, a in enumerate(self._pool.accounts):
                ...
                if not email:
                    continue
                # Blacklist gate: skip accounts flagged by genesis audit
                if email.lower() in blacklist:
                    continue
                # Pro Trial Only gate: skip Free accounts
                u = a.get('usage', {})
                plan = (u.get('plan') or '').lower()
                if plan == 'free':
                    continue
                # Skip long-expired accounts (planEnd + 3d < now)
                pe = u.get('planEnd', 0)
                if pe and pe > 1600000000000 and (pe + EXPIRED_GRACE_MS) < now_ms:
                    continue
    ...
    def pick_best_for_switch(self, exclude_email=None, model_key=None):
        """Pick best account for hot-switching (excludes current + remote-in-use)."""
        remote_in_use = _read_remote_in_use()
        with self.lock:
            candidates = []
            for state in self.accounts.values():
                if state.email == exclude_email:
                    continue
                if state.email.lower() in remote_in_use:
                    continue  # skip accounts being used on other machines
                if model_key and not state.available_for_model(model_key):
                    continue
                if not model_key and state.effective <= QUOTA_EXHAUSTED:
                    continue
    ...
    def _check(self):
        ...
        if should_switch:
            ...
            if best:
                ...
                if result['ok']:
                    print(f'[Guardian] OK Switched {result["ms"]}ms')
                else:
                    print(f'[Guardian] FAIL Switch failed')
