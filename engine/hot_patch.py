...
def _find_ext_js():
    ...
    for c in candidates:
        p = c / 'extensions' / 'windsurf' / 'dist' / 'extension.js'
        if p.exists():
            return p
    import glob
    for m in glob.glob(r'C:\Users\*\AppData\Local\Programs\Windsurf\resources\app\extensions\windsurf\dist\extension.js'):
        return Path(m)
    return candidates[0] / 'extensions' / 'windsurf' / 'dist' / 'extension.js'
...
def apply(ext_path: Path = EXT_JS, force: bool = False) -> bool:
    ...
    if not ext_path.exists():
        log(f'extension.js not found: {ext_path}', '[X] ')
        return False
    ...
    if PATCH_OLD not in src:
        log('Patch target not found in extension.js! (Windsurf updated?)', '[X] ')
    ...
    if count != 1:
        log(f'DANGER: Patch target appears {count} times (expected 1). Aborting.', '[X] ')
        return False

    # 备份
    backup = _backup(ext_path)
    log(f'Backup -> {backup.name}', '[BAK]')
    ...
    v = verify(ext_path)
    if v['patched']:
        log(f'Patch APPLIED OK  size={v["ext_size"]:,}  hash={v["ext_hash"]}', '[OK] ')
        log(f'Key file: {POOL_KEY_FILE}', '[F] ')
        log(f'Key valid: {v["pool_key_valid"]}', '[K] ')
        print()
        log('Windsurf needs ONE restart to activate the patch.', '[!] ')
        ...
        log('Patch write FAILED -- restoring backup', '[X] ')
...
def restore(ext_path: Path = EXT_JS) -> bool:
    ...
    if not BACKUP_DIR.exists():
        log('No backup directory found.', '[X] ')
        return False

    baks = sorted(BACKUP_DIR.glob('*.js.bak'))
    if not baks:
        log('No backups found.', '[X] ')
        return False
    ...
    if not target_bak:
        # 用最新备份
        target_bak = baks[-1]
        log(f'Warning: Using latest backup (may be patched): {target_bak.name}', '[!] ')
    else:
        log(f'Clean backup found: {target_bak.name}', '[OK] ')

    shutil.copy2(target_bak, ext_path)
    v = verify(ext_path)
    log(f'Restored: patched={v["patched"]}  hash={v["ext_hash"]}', '[OK] ')
    log('Windsurf needs ONE restart to deactivate the patch.', '[!] ')
    return True


def watch(ext_path: Path = EXT_JS, interval: int = 30):
    """监视 extension.js 变化 (Windsurf更新后自动重补丁)。"""
    log(f'Watching {ext_path.name} every {interval}s...', '[W] ')
        ...
        try:
            ...
            if h != last_hash:
                ...
                if not v['patched']:
                    ...
                    log('Already patched, skip.', '[OK] ')
        except Exception as e:
            log(f'Watch error: {e}', '[X] ')
...
def hot_test():
    ...
    icon = '[OK]' if v['patched'] else ('[!]' if v['original_present'] else '[FAIL]')
    ...
    try:
        ...
        print(f'  [OK] Engine :19877: {pool["total"]} accts, {pool["available"]} avail, D{pool["total_daily"]}% W{pool["total_weekly"]}%')
    except Exception as e:
        results['engine_ok'] = False
        print(f'  [FAIL] Engine :19877: {e}')
    ...
    try:
        ...
        print(f'  [OK] Proxy  :19876: running (proxy={d.get("proxy","?")})')
    except Exception as e:
        results['proxy_ok'] = False
        print(f'  [!] Proxy  :19876: not running ({e})')
    ...
    try:
        ...
        conn.close()
        print(f'  [API] apiServerUrl secret: {row[0]}B in state.vscdb (active only after restart)')
    except Exception as e:
        print(f'  [!] apiServerUrl check: {e}')
    ...
    try:
        ...
        active = d.get('active', {})
        if active:
            print(f'  [KEY] Active: #{active["index"]} {active["email"][:30]} D{active["daily"]}%-W{active["weekly"]}% score={active["score"]}')
    except Exception:
        pass
    ...
    try:
        ...
        d = json.loads(r.read())
        acct = d.get('account', {})
        print(f'  [PICK] Best for claude-sonnet: #{acct.get("index")} {acct.get("email","?")[:25]}')
    except Exception:
        pass
...
def write_pool_key_now():
    ...
    try:
        ...
        if best and best.api_key:
            ...
            log(f'Key file written: #{best.index} {best.email[:25]} {best.api_key[:20]}...', '[OK] ')
            return True
        else:
            log('No account with apiKey found', '[X] ')
            return False
    except Exception as e:
        log(f'write_pool_key_now error: {e}', '[X] ')
        return False
...
def start_pool_key_daemon():
    ...
    t.start()
    log(f'Pool key daemon started -> {POOL_KEY_FILE}', '[OK] ')
    return engine
...
def restore_api_server_url() -> bool:
    ...
    try:
        ...
        if backup:
            ...
            conn.commit()
            log('apiServerUrl restored to original (restart Windsurf to take effect)', '[OK] ')
            conn.close()
            return True
        else:
            log('No apiServerUrl backup found (not modified by proxy --setup)', '[!] ')
            conn.close()
            return False
    except Exception as e:
        log(f'apiServerUrl restore error: {e}', '[X] ')
        return False
...
def cli_full_restore():
    ...
    if ok1:
        log('Full restore complete. Restart Windsurf to deactivate.', '[OK] ')
    ...
    if cmd == 'apply':
        ...
        try:
            while True:
                ...
                best = eng.pick_best()
                if best:
                    print(f'  [KEY] Key -> #{best.index} {best.email[:25]} D{best.daily}%-W{best.weekly}%')
