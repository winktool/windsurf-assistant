...
class AccountPool:
    ...
    def get_health(self, acc):
        ...
        # protobuf3: 零值不序列化 → 字段缺失=0, 非100
        # 已检查过的账号: remaining缺失 → 0(耗尽); 未检查过: → -1(未知)
        fallback = 0 if last_checked else -1
        dr = d.get('remaining', fallback) if d else fallback
        wr = w.get('remaining', fallback) if w else fallback
...
# ============================================================
# Hot-Inject Switcher v2 (auth_bridge + legacy fallback)
# ============================================================
# v2: Official injection via provideAuthTokenToAuthProvider
#     Falls back to legacy DB-write if auth_bridge unavailable
try:
    from auth_bridge import switch_account as _bridge_switch, firebase_login as _bridge_firebase
    _HAS_AUTH_BRIDGE = True
except ImportError:
    _HAS_AUTH_BRIDGE = False

class HotSwitcher:
    """
    v2: auth_bridge path (official) > legacy DB-write path (fallback)
    auth_bridge: Firebase login → idToken → extension provideAuthTokenToAuthProvider
    legacy:      snapshot DB-write → window reload
    """
...
'''
    ...
    @staticmethod
    def hot_switch(target_email, store, retries=2, pool=None):
        """Execute hot-inject switch. Prefers auth_bridge (official), falls back to legacy.
        Returns (ok, steps, total_ms)."""
        t0 = time.time()
        steps = []

        # ── Path A: auth_bridge (official provideAuthTokenToAuthProvider) ──
        if _HAS_AUTH_BRIDGE and pool:
            password = None
            for acc in (pool.accounts if hasattr(pool, 'accounts') else []):
                if acc.get('email', '').lower() == target_email.lower():
                    password = acc.get('password')
                    break
            if password:
                steps.append('Path: auth_bridge (official)')
                ok, info = _bridge_switch(target_email, password, wait=True)
                ms = info.get('ms', 0)
                if ok:
                    steps.append(f'Inject: OK account={info.get("account","?")} via {info.get("channel","?")}')
                    try:
                        write_active_marker(target_email)
                    except Exception:
                        pass
                    steps.append(f'Total: {ms}ms')
                    return True, steps, ms
                else:
                    steps.append(f'auth_bridge failed: {info.get("error","?")} — falling back to legacy')

        # ── Path B: Legacy (snapshot DB-write + reload) ──
        snapshot = store.get_snapshot(target_email)
        if not snapshot:
            steps.append(f'No auth snapshot for {target_email}. Run: harvest')
            return False, steps, int((time.time() - t0) * 1000)

        ok, msg = HotSwitcher.inject_auth(snapshot)
        steps.append(f'Legacy inject: {msg}')
        if not ok:
            return False, steps, int((time.time() - t0) * 1000)
...
# ============================================================
def _read_proto_quota_from_vscdb():
    """v9.0: Read live quota directly from windsurfAuthStatus protobuf in state.vscdb.
    Returns: { daily, weekly, plan, email, resetTime, weeklyReset } or None.
    Proto3: absent field 15 in quota mode = 0% remaining (exhausted)."""
    ...
    def _pp(d):
        ...
        while p < len(d):
            try:
                t, p = _rv(d, p)
                fn = t >> 3; wt = t & 7
                if fn == 0 or fn > 10000: break
                if wt == 0:
                    v, p = _rv(d, p); f.setdefault(fn, []).append(('v', v))
                elif wt == 2:
                    l, p = _rv(d, p)
                    if p + l > len(d): break
                    f.setdefault(fn, []).append(('b', d[p:p+l])); p += l
                elif wt == 1: p += 8
                elif wt == 5: p += 4
                else: break
            except: break
    ...
    try:
        ...
        email = None
        if 7 in top and top[7][0][0] == 'b':
            try: email = top[7][0][1].decode()
            except: pass
        # field 13 = PlanStatus
        if 13 not in top or top[13][0][0] != 'b': return None
        ps = _pp(top[13][0][1])
        # PS[1] = PlanInfo → PI[2]=planName, PI[35]=billingStrategy
        plan_name, billing = None, None
        if 1 in ps and ps[1][0][0] == 'b':
            pi = _pp(ps[1][0][1])
            if 2 in pi and pi[2][0][0] == 'b':
                try: plan_name = pi[2][0][1].decode()
                except: pass
            if 35 in pi:
                bs = pi[35][0][1] if pi[35][0][0] == 'v' else None
                if bs == 1: billing = 'credits'
                elif bs == 2: billing = 'quota'
                elif bs == 3: billing = 'acu'
        is_quota = billing in ('quota', 'acu')
        # PS[14]=daily%, PS[15]=weekly% (proto3: absent=0 in quota mode)
        daily = ps[14][0][1] if 14 in ps and ps[14][0][0] == 'v' else (0 if is_quota else None)
        weekly = ps[15][0][1] if 15 in ps and ps[15][0][0] == 'v' else (0 if is_quota else None)
        # PS[17]=daily_reset_unix, PS[18]=weekly_reset_unix
        d_reset = ps[17][0][1] * 1000 if 17 in ps and ps[17][0][0] == 'v' else 0
        w_reset = ps[18][0][1] * 1000 if 18 in ps and ps[18][0][0] == 'v' else 0
        if d_reset and w_reset and w_reset > d_reset:
            w_reset = d_reset
        return {
            'daily': daily, 'weekly': weekly,
            'plan': plan_name, 'email': email,
            'daily_reset': d_reset, 'weekly_reset': w_reset,
        }
    except Exception as e:
        return None

def read_current_health():
    """Read current active account's health.
    v9.0: Dual-path — Login Helper data + live vscdb protobuf overlay.
    The vscdb protobuf is the ground truth for the ACTIVE account's quota."""
    pool = AccountPool()
    active_i = find_active_index(pool)
    if active_i < 0:
    ...
    # v9.0: Overlay live quota from vscdb protobuf (ground truth for active account)
    live = _read_proto_quota_from_vscdb()
    if live and live.get('daily') is not None:
        h['daily'] = live['daily']
        h['weekly'] = live['weekly'] if live.get('weekly') is not None else h['weekly']
        if live.get('plan'): h['plan'] = live['plan']
        if live.get('daily_reset'): h['daily_reset'] = live['daily_reset']
        if live.get('weekly_reset'): h['weekly_reset'] = live['weekly_reset']
        # Recalculate reset-in-seconds
        now_ms = time.time() * 1000
        h['daily_reset_in_sec'] = max(0, round((h['daily_reset'] - now_ms) / 1000)) if h['daily_reset'] > now_ms else 0
        h['weekly_reset_in_sec'] = max(0, round((h['weekly_reset'] - now_ms) / 1000)) if h['weekly_reset'] > now_ms else 0
    ...
    def do_GET(self):
        try:
            ...
            # ── Watchdog compatibility routes (/api/pool/*) ──
                ...
                self._json({
                    'ok': True,
                    'dailyQuotaPercent': h.get('daily', -1),
                    'weeklyQuotaPercent': h.get('weekly', -1),
                    'dPercent': h.get('daily', -1),
                    'wPercent': h.get('weekly', -1),
                    'rateLimited': s.get('rate_limited', 0) > 0,
                    'isRateLimited': s.get('rate_limited', 0) > 0,
                    'quotaExhausted': (h.get('daily', -1) <= 5 or h.get('weekly', -1) <= 5),
                    'dailyQuotaExhausted': h.get('daily', -1) <= 5,
