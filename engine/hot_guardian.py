...
EXT_JS      = Path(r'E:\Windsurf\resources\app\extensions\windsurf\dist\extension.js')
...
class HotGuardian:
    ...
    def _heartbeat(self):
        e = 'OK' if engine_port_alive() else 'FAIL'
        pr = 'OK' if proxy_port_alive() else 'FAIL'
        pa = 'OK' if self._patch_watcher.is_patched() else 'FAIL'
        ks = self._key_keeper.status()
        kk = 'OK' if ks['key_valid'] else 'FAIL'
