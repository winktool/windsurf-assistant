...
def _is_pro_trial(account):
    """Pro Trial Only gate: returns True only for valid Pro Trial accounts."""
    u = account.get('usage') or {}
    plan = (u.get('plan') or '').lower()
    if plan == 'free':
        return False
    # Grace period expired → degraded to free
    gps = u.get('gracePeriodStatus')
    if gps is not None and gps > 1:
        bs = (u.get('billingStrategy') or '').lower()
        if not bs or bs == 'free':
            return False
    # Long-expired (planEnd + 3d < now)
    pe = u.get('planEnd', 0)
    if pe and pe > 1.6e12 and (pe + 3 * 86400000) < time.time() * 1000:
        return False
    return True
...
def inject_to_vsix(account_data):
    ...
    Pro Trial Only: rejects Free and long-expired accounts."""
    # Pro Trial gate
    if not _is_pro_trial(account_data):
        log(f"拒绝注入: {account_data.get('email','?')[:30]} — 非Pro Trial", False)
        return False
...
def inject_all_to_vsix():
    ...
    # Pro Trial Only filter
    pro_trial = [a for a in real if _is_pro_trial(a)]
    skipped = len(real) - len(pro_trial)
    if skipped:
        log(f"过滤 {skipped} 个非Pro Trial账号", False)
    real = pro_trial

    log(f"准备注入 {len(real)} 个Pro Trial账号到无感切号...")
    injected = 0
    for a in real:
        if inject_to_vsix(a):
            injected += 1
...
def main():
    ...
    if cmd == 'status':
        ...
        # Delegate to standalone purge script
        purge_script = PROJECT_ROOT / '030-额度_Credits' / 'engine' / '_purge_now.py'
        if purge_script.exists():
            subprocess.run(['python', str(purge_script)], cwd=str(purge_script.parent))
        else:
            log(f"Purge脚本不存在: {purge_script}", False)
