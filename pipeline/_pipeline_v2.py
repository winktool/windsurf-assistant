...
LH_PATHS = [
    r'C:\\Users\\YOUR_USER\AppData\Roaming\Windsurf\User\globalStorage\undefined_publisher.windsurf-login-helper\windsurf-login-accounts.json',
    r'C:\\Users\\YOUR_USER\AppData\Roaming\Windsurf\User\globalStorage\windsurf-login-accounts.json',
]
...
def show_status():
    ...
    for a in accts:
        ...
        d = u.get("daily", {})
        w = u.get("weekly", {})
        lc = u.get("lastChecked", 0)
        _fb = 0 if lc else -1
        dr = d.get("remaining", _fb) if d else _fb
        wr = w.get("remaining", _fb) if w else _fb
        d_list.append(dr)
        w_list.append(wr)
        eff = min(dr, wr)
        if eff > 5: healthy += 1
