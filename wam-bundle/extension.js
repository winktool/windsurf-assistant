...
async function cleanupThirdPartyState() {
  ...
  } catch (e) { log(`cleanup: logout skipped: ${e.message}`); }
  // 停止所有引擎
  _stopEngines();
  if (_pollTimer) { clearInterval(_pollTimer); _pollTimer = null; }
  // v7.4: 停止心跳定时器
  if (_heartbeatTimer) { clearInterval(_heartbeatTimer); _heartbeatTimer = null; log('cleanup: heartbeat stopped'); }
  // v7.4: 停止文件监听
  if (_watcher) { _watcher.close(); _watcher = null; log('cleanup: watcher stopped'); }
  ...
  try {
    ...
    if (fs.existsSync(markerPath)) { fs.unlinkSync(markerPath); cleaned++; }
  } catch {}
  ...
  log(`cleanup: ${cleaned} items cleaned, engines+heartbeat+watcher stopped, session logged out — 回归本源`);
  return cleaned;
}
...
function _restartBackgroundServices() {
  ...
  if (!_heartbeatTimer) {
    _heartbeatTimer = setInterval(() => {
      if (!isWamMode()) return;
      const activeAcc = _store && _store.activeIndex >= 0 ? _store.get(_store.activeIndex) : null;
...
async function firebaseLogin(email, password) {
  ...
  const retryPromises = channels.map(ch =>
    _firebaseVia(ch, email, password, retryKey)
      .then(r => {
        if (r && r.idToken) return { ok: true, idToken: r.idToken, channel: `retry-${ch}-${retryKeySuffix}` };
        throw new Error('no_token');
      })
      .catch(e => { errors[`retry-${ch}`] = e.message; throw e; })
  );
  try {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      if (result && result.error) {
        log(`inject: attempt ${attempt} error: ${JSON.stringify(result.error)}`);
      } else {
        log(`inject: attempt ${attempt} unexpected: ${JSON.stringify(result || null).substring(0, 200)}`);
      }
    } catch (e) {
...
function _extractInjectResult(result) {
  ...
  if (result.session) {
    ...
    return { ok: true, account: s.account?.label || '?', apiKey: s.accessToken || '', sessionId: s.id || '' };
  }
  if (result.account && !result.error) {
    return { ok: true, account: result.account?.label || result.account || '?', apiKey: result.accessToken || '', sessionId: result.id || '' };
  }
  return null;
}
...
async function switchToAccount(email, password) {
  ...
  if (_prewarmedToken && _prewarmedToken.email === emailKey && (Date.now() - _prewarmedToken.ts) < TOKEN_CACHE_TTL) {
    idToken = _prewarmedToken.idToken;
    log(`switch: ⚡ pre-warmed token HIT (${Math.round((Date.now() - _prewarmedToken.ts)/1000)}s old) [${Date.now()-t0}ms]`);
  ...
  if (!idToken) {
    ...
    if (!loginResult.ok) {
      ...
      if (/INVALID|NOT_FOUND|DISABLED|WRONG/.test(err)) {
        const idx = _store?.accounts.findIndex(a => a.email.toLowerCase() === emailKey);
        if (idx >= 0) {
    ...
    _tokenCache.set(emailKey, { idToken, expiresAt: Date.now() + TOKEN_CACHE_TTL });
    log(`switch: login OK via ${loginResult.channel} (${idToken.length}ch) [${Date.now()-t0}ms]`);
  }
...
async function _prewarmCandidateToken(candidateIndex) {
  ...
  // 如果已有有效缓存, 不重复预热
  const cached = _tokenCache.get(emailKey);
  if (cached && cached.expiresAt > Date.now() + 300000) { // 至少还有5分钟有效期
    _prewarmedToken = { email: emailKey, idToken: cached.idToken, ts: cached.expiresAt - TOKEN_CACHE_TTL };
  ...
  try {
    ...
    const loginResult = await firebaseLogin(acc.email, acc.password);
    if (loginResult.ok) {
      _prewarmedToken = { email: emailKey, idToken: loginResult.idToken, ts: Date.now() };
      _tokenCache.set(emailKey, { idToken: loginResult.idToken, expiresAt: Date.now() + TOKEN_CACHE_TTL });
      log(`🔥 prewarm: OK for ${acc.email.substring(0, 20)} (${loginResult.idToken.length}ch)`);
    } else {
      log(`🔥 prewarm: FAIL for ${acc.email.substring(0, 20)}: ${loginResult.error}`);
    }
  } catch (e) {
      ...
      if (hasFluctuation) {
        ...
        if (autoRotate && !_switching && !switchCooldown) {
          ...
          if (bestI >= 0) {
            ...
            _switching = true; _switchingStartTime = Date.now();
            try {
          ...
          if (bestI >= 0) {
            ...
            try {
              ...
              if (sr.ok) {
                ...
                _lastSwitchTime = Date.now();
                _predictiveCandidate = _store.getBestIndex(bestI, true);
                if (_predictiveCandidate >= 0) _prewarmCandidateToken(_predictiveCandidate); // v8: 预热下一个
                _store.save();
                _quotaSnapshots.delete(bestAcc.email.toLowerCase());
                _snapshotDirty = true; _schedulePersist();
                _burstUntil = Date.now() + BURST_DURATION;
...
function updateStatusBar() {
  ...
  if (_mode === 'official') {
    ...
    _statusBarItem.tooltip = 'WAM v8.0 [官方模式] — 所有切号功能已停止\n点击打开管理面板，可切回WAM模式';
    return;
  }
  ...
  if (activeAcc) {
    ...
    _statusBarItem.tooltip = `WAM v8.0 [WAM切号]${s.drought ? ' [🏜️Weekly干旱模式·只看D]' : ''}\n` +
      `活跃: ${activeAcc.email}\n${h.plan}\n` +
    ...
    _statusBarItem.tooltip = `WAM v8.0 [WAM切号] · 未选择活跃账号\n日重置: ${s.hrsToDaily.toFixed(1)}h后 · 周重置: ${s.hrsToWeekly.toFixed(1)}h后`;
  }
}
...
async function handleWebviewMessage(msg) {
  switch (msg.type) {
    case 'switch': {
      ...
      // v7.4: 官方模式下不自动翻转
      if (_mode === 'official') {
        vscode.window.showWarningMessage('WAM: 官方模式下无法切号，请先切回WAM模式');
        return;
      }
      ...
      if (_switching) {
        ...
        if (lockAge < 30000) {
          vscode.window.showWarningMessage(`WAM: 正在切换中(${Math.round(lockAge/1000)}s)...请稍候`);
          return;
        }
        log(`switch: 手动抢占 — 强制释放超时锁(${Math.round(lockAge/1000)}s)`);
        _switching = false;
      }
      _switching = true; _switchingStartTime = Date.now();
    ...
    case 'autoRotate': {
      ...
      if (!isWamMode()) {
        vscode.window.showWarningMessage('WAM: 官方模式下智能轮转已禁用，请先切回WAM模式');
        break;
      }
    ...
    case 'setMode': {
      ...
      if (newMode === 'wam') {
        ...
        if (_store.activeIndex < 0 && _store.pwCount() > 0) {
          ...
          if (bestI >= 0) {
            ...
            if (_switching) {
              log(`setMode: 强制释放锁(${Math.round((Date.now()-_switchingStartTime)/1000)}s)`);
              _switching = false;
            }
            _switching = true; _switchingStartTime = Date.now();
            try {
      ...
      if (newMode === 'official') {
        ...
        vscode.window.showInformationMessage(`WAM: 官方模式 — WAM会话已登出，${cleaned}项清理完成。请用Windsurf原生登录您自己的账号`);
      }
      refreshAll();
      break;
    }
...
async function doAutoRotate(store) {
  ...
  if (_switching) {
    ...
    log(`autoRotate: 手动抢占 — 强制释放超时锁(${Math.round(lockAge/1000)}s)`);
    _switching = false;
  }
  _switching = true; _switchingStartTime = Date.now();
  try {
  ...
  _watcher = fs.watch(WAM_DIR, (eventType, filename) => {
    if (filename === 'oneshot_token.json' && eventType === 'rename') {
      setTimeout(async () => {
        ...
        } catch { return; } // rename失败 = 其他实例已抢走
        if (!isWamMode()) { log('watcher: skip injection (official mode)'); return; }
        try {
...
function activate(context) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('wam.switchAccount', async () => {
      ...
      if (_mode === 'official') {
        const choice = await vscode.window.showWarningMessage(
          'WAM: 当前为官方模式，切号将切回WAM模式。确认？', '切回WAM模式', '取消');
      ...
      if (pick) {
        ...
        if (_switching) {
          ...
          log(`switchAccount: 手动抢占 — 强制释放超时锁(${Math.round(lockAge/1000)}s)`);
          _switching = false;
        }
        _switching = true; _switchingStartTime = Date.now();
        try {
    ...
    vscode.commands.registerCommand('wam.panicSwitch', async () => {
      ...
      if (_mode === 'official') {
        const choice = await vscode.window.showWarningMessage(
          'WAM: 官方模式下紧急切换将切回WAM模式', '确认紧急切换', '取消');
      ...
      if (_switching) {
        log(`panicSwitch: 强制释放锁(${Math.round((Date.now()-_switchingStartTime)/1000)}s)`);
        _switching = false;
      }
      _switching = true; _switchingStartTime = Date.now();
      try {
    ...
    vscode.commands.registerCommand('wam.officialMode', async () => {
      ...
      saveMode('official');
      const cleaned = await cleanupThirdPartyState();
      vscode.window.showInformationMessage(`WAM: 官方模式已激活 — WAM会话已登出，${cleaned}项清理完成。请使用Windsurf原生登录您自己的账号`);
      refreshAll();
    }),
    vscode.commands.registerCommand('wam.wamMode', async () => {
      ...
      if (_store.activeIndex < 0 && _store.pwCount() > 0) {
        ...
        if (bestI >= 0) {
          const acc = _store.get(bestI);
          if (_switching) {
            log(`wamMode: 强制释放锁(${Math.round((Date.now()-_switchingStartTime)/1000)}s)`);
            _switching = false;
          }
          _switching = true; _switchingStartTime = Date.now();
          try {
    ...
    vscode.commands.registerCommand('wam.status', () => {
      ...
      let msg = `WAM v8.0.0 | ${stats.pwCount}号 D${stats.totalD}·W${stats.totalW} | mode=${_mode} | 监测${_totalMonitorCycles}轮·${_totalChangesDetected}次变动·${_store.switchCount}次切号 | inst=${_instanceId}`;
  ...
  try {
    ...
    const _rlInterceptor = vscode.workspace.onDidChangeTextDocument(async (e) => {
      if (!isWamMode() || _switching || !_store || _store.activeIndex < 0) return;
      ...
      if (/rate.?limit.?exceeded|Rate limit error/i.test(newText)) {
        ...
        log(`🚨 rate-limit intercepted in document! Triggering proactive switch...`);
        const autoRotate = vscode.workspace.getConfiguration('wam').get('autoRotate', true);
        if (!autoRotate) return;
        const bestI = _predictiveCandidate >= 0 ? _predictiveCandidate : _store.getBestIndex(_store.activeIndex, true);
        if (bestI < 0) { log('rate-limit: no available account'); return; }
        const bestAcc = _store.get(bestI);
        _switching = true; _switchingStartTime = Date.now();
        try {
          const sr = await switchToAccount(bestAcc.email, bestAcc.password);
          if (sr.ok) {
            ...
            _lastSwitchTime = Date.now();
            _predictiveCandidate = _store.getBestIndex(bestI, true);
            if (_predictiveCandidate >= 0) _prewarmCandidateToken(_predictiveCandidate);
            _store.save();
            _quotaSnapshots.delete(bestAcc.email.toLowerCase());
            _snapshotDirty = true; _schedulePersist();
            vscode.window.showInformationMessage(`WAM: 🚨 Rate-limit拦截 → 已无感切换到 ${sr.account} (${sr.ms}ms)`);
            refreshAll();
          }
        } finally { _switching = false; }
      }
  ...
  setTimeout(() => {
    ...
    if (isWamMode()) {
      ...
      log('startup: WAM模式 — 纯切号·监测引擎待首次切号后自启动');
    } else {
...
function deactivate() {
  ...
  if (_rateLimitWatcher) { _rateLimitWatcher.dispose(); _rateLimitWatcher = null; }
